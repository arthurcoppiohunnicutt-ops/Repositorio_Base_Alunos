...
# Calcule a media
# informe se o aluno esta aprovado (media>=7), recuperacao(5<media<7) ou reprovado
# Apresente as notas que o aluno tirou, a media e o status de sua situacao
...
for i in range(4): # faz a pergunta 4 vezes
    ...
    if nota > 0: # so aceita nota positiva
        notas.append(nota) # notas.append(notaa) adiciona um elemento no final da lista
    else: # se a nota for negativa apresenta o print
        print('Valor inválido.')
print(f'Suas notas são: {notas}')  # linha opcional

media = sum(notas) / len(notas) # a funcao sun(notas) soma todas as notas da lista
# a funcao len(notas) informa o tamanho da lista nostas
